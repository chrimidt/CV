...
%-------------------------------------------------
\begin{frame}{Background}
  \textbf{Multi-label Learning From Single Positive Labels}
  \begin{itemize}
    \item One image - multiple labels - only one positive label.
    \item All other labels are unobserved, not confirmed negatives.
    \item Binary Cross-Entropy (BCE) fails: missing label is assumed negative - false negatives.
  \end{itemize}
\end{frame}
...
%         \item Guides parameter optimization by minimizing the loss.
%       \end{itemize}
%     \end{column}
...
%     \item $\mathbf{F}\in[0,1]^{N\times K}$: matrix of classifier predictions.
%     \item $\mathbf{\tilde{Y}}\in[0,1]^{N\times K}$: matrix of estimated labels.
%     \item $\mathbf{Z}_B$: fixed partial labels. 
...
\begin{frame}{Background}
  ...
  \begin{itemize}
    \item Jointly train a classifier $f$ and a label estimator $g$.
    \item Missing labels are treated as unknown - a separate label estimator assigns them soft probabilities.
    \item Enforce expected number of positives via batch-level regularization.
  \end{itemize}
  \begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{Images/role2.PNG}
    \caption*{Alternating updates of image classifier (f) and label estimator (g).}
  \end{figure}
\end{frame}

%-------------------------------------------------
\begin{frame}{Background}
   ROLE: loss function tailored for sparse supervision by alternating updates:
  \[
  ...
  \]
  \begin{itemize}
    \item $\mathcal{L}_{BCE}$ term: encourages the classifier predictions $\mathbf{F}_B$ to match the estimated labels $\tilde{\mathbf{Y}}_B$.
    \item  $\mathcal{L}_{EPR}$ term: enforces correct predictions for known positives and penalizes deviations from the expected number of positives per image.
  \end{itemize}
  Here, $\mathcal{L}_{EPR}$ is given by:
  \[
    \mathcal{L}_{\text{EPR}}(\mathbf{F}_B,\mathbf{Z}_B) = \frac{1}{|B|}\sum_{n\in B} \mathcal{L}_{\text{BCE}}^{+}(\mathbf{f}_n,\mathbf{z}_n) + \lambda\bigl(\frac{\hat{\kappa}(\mathbf{F}_B) - \kappa}{K}\bigr)^2
  \]
\end{frame}
...
\begin{frame}{Background}
  ...
  \end{equation*}
  where $\mathbf{F}_B$ and $\tilde{\mathbf{Y}}_B$ are updated simultaneously.
\end{frame}
